...
class UsuarioRepo:
    ...
    def inserir(cls, usuario: Usuario) -> Optional[Usuario]:
        try:
            with criar_conexao() as conexao:
                cursor = conexao.cursor()
                cursor.execute(SQL_INSERIR, (usuario.nome, usuario.email, usuario.senha, usuario.admin),)
    ...
    def alterar(cls, usuario: Usuario) -> bool or False:
        with criar_conexao() as conexao:
            cursor = conexao.cursor()
            cursor.execute(
            SQL_ALTERAR, (usuario.nome, usuario.email, usuario.admin, usuario.id,),)
            return cursor.rowcount > 0
    ...
    def obter_todos(cls) -> List[Usuario]:
        with criar_conexao() as conexao:
            ...
            objetos = [Usuario(id=t[0], nome=t[1], email=t[2], admin=t[3]) for t in tuplas]
            return objetos
    ...
    def alterar_senha(cls, usuario: Usuario) -> bool or False:
        with criar_conexao() as conexao:
            cursor = conexao.cursor()
            cursor.execute(
            SQL_ALTERAR_SENHA_EMAIL, (usuario.senha, usuario.email ))
            return cursor.rowcount > 0